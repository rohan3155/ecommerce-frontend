    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post('https://ecommerce-backend-556q.onrender.com/Admin/updateProduct', body).then(res => {
                ...
                } else if (res.data.response === "Succesfully Updated Product") {
    ...
    return (
        ...
        <div className="flex flex-col justify-center w-4/5 px-4 py-4 overflow-hidden">
            <div className="w-full p-6 m-auto bg-white rounded-md shadow-md ring-2 ring-gray-800/50 lg:max-w-xl 2xl:max-w-4xl">
                <h1 className="text-3xl font-semibold text-center text-gray-700">Update Product</h1>
                <Link to={'/admin/viewProduct'}><button className='btn btn-success'> ←Go Back</button></Link>
                ...
                >
                        ...
                        {!loading2 && (
                            <div>
                                <select className="w-full my-2 select select-bordered" onChange={e => setSCategoryName(e.target.value)} >
                                    <option defaultValue={CategoryName}  disabled>{CategoryName}</option>
                        ...
                        {!loading1 && (
                            <div>
                                <select className="w-full my-2 select select-bordered" onChange={e => setSSubCategoryName(e.target.value)} >
                                    ...
                                    <option defaultValue={SubCategoryName}  disabled>{SubCategoryName}</option>


                                    {UData1.map((item, key, index) =>
                    ...
                    <div>
                        <button className="btn btn-block btn-primary">Update Product</button>
                    </div>
