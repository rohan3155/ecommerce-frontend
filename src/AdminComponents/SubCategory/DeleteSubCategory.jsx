...
const body = {
      ...
      try {
          await axios.post('https://ecommerce-backend-556q.onrender.com/Admin/deleteSubCategory', body).then(res => {
              ...
              if (res.data === "No item found") {
                  ...
                  history(`/admin/deletesubcategory`)
                  setTimeout(() => {
                    // e.preventDefault();
                    location. reload()
                }, 800);
              } else if (res.data.response === "Subcategory Deleted Successfuly") {
  ...
  return (
    ...
    <h1 className="text-3xl font-semibold text-center text-gray-700">Are you Sure You want to delete this SubCategory?</h1>
    <Link to={'/admin/viewsubcategory'}><button className='btn btn-success'> ← No ,Go Back</button></Link>
        <form className="space-y-4" method='post'
         ...
         >
                    ...
                    <div>
                        <button className="btn btn-block btn-error">Delete SubCategory</button>
                    </div>
