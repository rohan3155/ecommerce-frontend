...
const body = {
    ...
    data:{firstname, lastname, phone, email, password},
      ...
      try {
          await axios.post('https://ecommerce-backend-556q.onrender.com/Admin/deleteUser', body).then(res => {

              if (res.data === "No item found") {

                  history(`/admin/DeleteUser`)
                  setTimeout(() => {
                    // e.preventDefault();
                    location. reload()
                }, 800);
              } else if (res.data.response === "User Deleted Successfuly") {
  ...
  return (
     ...
     <div className="flex flex-col justify-center w-full px-4 py-4 overflow-hidden">
        <div className="w-full p-6 m-auto bg-white rounded-md shadow-md ring-2 ring-gray-800/50 lg:max-w-xl 2xl:max-w-4xl">
            <h1 className="text-3xl font-semibold text-center text-gray-700">Are you Sure You want to delete this User?</h1>
            <Link to={'/admin/viewuser'}><button className='btn btn-success'> ← No ,Go Back</button></Link>
            <form className="space-y-4" method='post'
             ...
             >
                        ...
                        <div>
                            ...
                            <button className="btn btn-block btn-error">Delete User</button>
                        </div>
